...
def add_total_prob(func):
    def wrapper(*args, **kwargs):
        result = func(*args, **kwargs)  # Call the original sampling function
        wrapper.total_prob = wrapper._total_prob  # Store the additional data set in sampling
...
def sample(args, device, generative_model, dataset_info,
    ...
    if args.probabilistic_model == 'diffusion':
        ...
        # ------------------------------------------------------
        # Check if the model is wrapped in DataParallel, and handle it accordingly
        
        if isinstance(generative_model, torch.nn.DataParallel) or isinstance(generative_model, DDP):
            x, h = generative_model.module.sample(batch_size, max_n_nodes, node_mask, edge_mask, context, fix_noise=fix_noise)
            sample._total_prob = generative_model.module.get_total_prob()
        else:
            x, h = generative_model.sample(batch_size, max_n_nodes, node_mask, edge_mask, context, fix_noise=fix_noise)
            sample._total_prob = generative_model.get_total_prob()
