...
def process_xyz_files(data, process_file_fn, file_ext=None, file_idx_list=None, stack=True, pt_file_path=None, split=None):
    """
    ...
    molecules_ = torch.load(pt_file_path)

    # -------------------------------------------------------------
    # print(len(molecules_))
    # print(molecules_[0])
    # assert False, 'Terminate codes!'
    # -------------------------------------------------------------

    if file_idx_list is not None:
        molecules = [molecule for idx, molecule in enumerate(molecules_) if idx in file_idx_list]
    
    # save smiles
    smiles_list = [molecule.pop('smiles') for molecule in molecules]
    smiles_path = os.path.join(pt_file_path.split('/trainData')[0], 'smiles')
    smiles_file = pt_file_path.split('/')[-1].replace('.pt', f'_{split}.smi')
    smiles_file_path = os.path.join(smiles_path, smiles_file)
    if not os.path.exists(smiles_file_path):
        print('Getting smiles...')
        with open(smiles_file_path, 'w') as f:
            for smiles in smiles_list:
                f.write(smiles + '\n')
        print(f'SMILES saving at {smiles_file_path}')
...
if __name__ == "__main__":
    pt_file_path = "../DrugDesign/datasets/trainData/qm9_removeH_True.pt"
    process_xyz_files(None, None, file_ext=None, file_idx_list=None, stack=True, pt_file_path=pt_file_path)
