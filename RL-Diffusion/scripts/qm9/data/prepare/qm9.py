...
# from process import process_xyz_files, process_xyz_gdb9
# from utils import download_data, is_int, cleanup_file


def download_dataset_qm9(datadir, dataname, splits=None, calculate_thermo=False, exclude=True, cleanup=True, pt_file_path=None):
    """
    ...
    if splits is None:
        # splits = gen_splits_gdb9(gdb9dir, cleanup)pt_file_path
        splits = gen_splits_gdb9(pt_file_path, cleanup)
    ...
    for split, split_idx in splits.items():
        ...
        gdb9_data[split] = process_xyz_files(
            None, None, file_idx_list=split_idx, stack=True, pt_file_path=pt_file_path, split=split)
...
def gen_splits_gdb9(pt_file_path, cleanup=True):
    """
    ...
    Nmols = len(torch.load(pt_file_path))
    included_idxs = np.array(sorted(list(set(range(Nmols)))))

    Ntrain = int(0.8*Nmols)
    Ntest = int(0.1*Nmols)
    Nvalid = Nmols - (Ntrain + Ntest)

    print('Num_train:', Ntrain)
    print('Num_test:', Ntest)
    print('Num_valid:', Nvalid)
...
if __name__ == "__main__":
    root = '../DrugDesign/datasets/trainData_with_smiles'
    pt_file_paths = [
        'geom_removeH_False.pt',
        'geom_removeH_True.pt',
        'pubchem_removeH_False.pt',
        'pubchem_removeH_True.pt',
        'qm9_removeH_False.pt',
        'qm9_removeH_True.pt',
        'zinc15_removeH_False.pt',
        'zinc15_removeH_True.pt'
    ]
    for pt_file_path in pt_file_paths:
        pt_file_path = os.path.join(root, pt_file_path)
        download_dataset_qm9(datadir=None, dataname=None, splits=None, calculate_thermo=False, exclude=True, cleanup=True, pt_file_path=pt_file_path)