...
def check_stability(positions, atom_type, dataset_info, debug=False):
    ...
    for atom_type_i, nr_bonds_i in zip(atom_type, nr_bonds):
        ...
        if dataset_info['with_h'] == False:
            if type(possible_bonds) == int:
                is_stable = possible_bonds >= nr_bonds_i > 0
            else:
                is_stable = 0 < nr_bonds_i <= max(possible_bonds)
...
# ------------------------------------------------
def add_novel_molecules(func):
    def wrapper(*args, **kwargs):
        result = func(*args, **kwargs)  # Call the original sampling function
        wrapper.novel_molecules = wrapper._novel_molecules  # Store the additional data set in sampling
        return result  # Return the original result
    return wrapper
# ------------------------------------------------

@add_novel_molecules
def analyze_stability_for_molecules(molecule_list, dataset_info, RL=False, k=1, hyper_paras=None):
    ...
    if use_rdkit:
        ...
        if RL:
            rdkit_metrics = metrics.evaluate(processed_list, RL, k=k, hyper_paras=hyper_paras)
            analyze_stability_for_molecules._novel_molecules = {}  # for placeholder 
            return validity_dict, rdkit_metrics
        else:
            rdkit_metrics = metrics.evaluate(processed_list)
            # ------------------------------------------------
            analyze_stability_for_molecules._novel_molecules = metrics.get_novel_molecules()
            # ------------------------------------------------
