...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    # ------------------------------------------------------
    # def compute_log_prob_p_zs_given_zt(self, s, t, zt, zs, node_mask, edge_mask, context):
    #     """Compute log probability of zs ~ p(zs | zt)."""
    ...
    #     # Neural net prediction (eps_t).
    #     eps_t = self.phi(zt, t, node_mask, edge_mask, context)

    #     # Compute mean (mu) for p(zs | zt).
    ...
    #     # Log probability of zs given mu and sigma (Gaussian distribution).
    #     log_prob = -0.5 * (((zs - mu) ** 2) / sigma**2).sum(dim=-1)  # Sum over last dimension
    #     log_prob += -0.5 * torch.log(2 * torch.pi * sigma**2).sum(dim=-1)  # Gaussian normalization term
    ...
    @torch.no_grad()
    def sample(self, n_samples, n_nodes, node_mask, edge_mask, context, fix_noise=False, adjust_trajectory=None):
        """
        ...
        for s in reversed(range(0, self.T)):
            ...
            # self.z_t_list.append(z.detach().clone())   #####

            # if adjust_trajectory is not None:
                
            #     action = adjust_trajectory(z, s, node_mask) 
            #     z = z + action
    ...
    #     # ------------------------------------------------------
    #     total_log_prob = torch.zeros(n_samples, device=z.device)
    #     total_log_prob.requires_grad_(False) # original is True
    #     # ------------------------------------------------------

    #     # Iteratively sample p(z_s | z_t) for t = 1, ..., T, with s = t - 1.
    #     def checkpointed_sampling(z, total_log_prob):
    #         for s in reversed(range(0, self.T)):
    #             s_array = torch.full((n_samples, 1), fill_value=s, device=z.device)
    #             t_array = s_array + 1
    #             s_array = s_array / self.T
    ...
    #         # ------------------------------------------------------
    #             z_next = self.sample_p_zs_given_zt(s_array, t_array, z, node_mask, edge_mask, context, fix_noise=fix_noise)

    #             self.z_t_list.append(z_next)  # added march 11

    #             ##### added start
    #             if adjust_trajectory is not None:  # 允许外部调整轨迹
    #                 # action = adjust_trajectory(z, s)
    #                 action = adjust_trajectory(z, s, node_mask)
    #                 z_next = z_next + action
    #                 z_next[:, :, :self.n_dims] = diffusion_utils.remove_mean_with_mask(z_next[:, :, :self.n_dims], node_mask)
    #             ##### added end
    #             log_prob_step = self.compute_log_prob_p_zs_given_zt(s_array, t_array, z_next, z, node_mask, edge_mask, context)
    #             # total_log_prob += log_prob_step
    #             total_log_prob = total_log_prob + log_prob_step
    #             z = z_next
    #         return z, total_log_prob
        
    #     z, total_log_prob = checkpoint.checkpoint(checkpointed_sampling, z, total_log_prob, use_reentrant=True)

    #     # self.total_log_prob = total_log_prob
    #     self.total_log_prob = total_log_prob / self.T
    #     # ------------------------------------------------------
